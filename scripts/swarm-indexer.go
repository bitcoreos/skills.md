...
// ID derivation: sources/provider/skill.js -> provider.skill
...
// Also keep main index for backward compat if needed, or remove it.
// For now, let's keep swarm/index.json as the full list for small swarms
os.WriteFile("swarm/index.json", jsonData, 0644)
} else {
fmt.Println("Index < 5MB. Writing monolithic index.")
os.WriteFile("swarm/index.json", jsonData, 0644)
// Clean up shards if they exist to avoid confusion?
// os.RemoveAll("swarm/index") // Optional
}
